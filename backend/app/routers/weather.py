...
@router.get("/current")
async def current_weather(location: str = Query(..., description="City, zip code, coordinates, or landmark")):
...
@router.get("/forecast")
async def forecast(location: str = Query(..., description="City, zip code, coordinates, or landmark")):
    geo = await resolve_location(location)
